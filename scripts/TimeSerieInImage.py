...
class TimeSerieInImage(object):
    ...
    def step(self, frame, value):
        ...
        y_pre  = self.data_serie[0:min(self.num_points_to_plot, self.data_serie.size)]
            # max(0, frame_index - self.num_points_to_plot):frame_index + 1, ns].copy()
        y_pre = 1. - y_pre
        ...
        # print(len(x_pre), len(y_pre), len(xy_pre))
        # print(len(x_post), len(y_post), len(xy_post))
        ...
        # bar_width = int(float(width) * min(max(self.predictions[frame_index][0], 0.0), 1.0))
        # bar_img = Image.new('RGBA', (bar_width, 20), color="yellow")
        # img.paste(bar_img, (0, height))
